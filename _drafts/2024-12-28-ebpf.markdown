...
The next step is to write the kernel-space part of the eBPF program, the file will be named `hello-world.bpf.c`, note the `.bpf` to indicate that it is a bpf program written in C, not a regular user space program.
...
```bash
clang -target bpf -c hello-world.bpf.c -O2 -g
```
...
If we now inspect the generated binary, named `hello-world.bpf.o`

```bash
readelf -S hello-world.bpf.o 
...
Now we only have the necessary sections present for the eBPF program to be loaded into kernel, we can write the user-space part of the program. Working with eBPF programs from the user-space is sometimes repetetive and alot of boilerplate code needs to be written. Luckily, bpftool provides us with a lot of usefull commands to work with eBPF programs, one of which is to dump a C header file which we then can import in out user-space program that takes care of alot of the boilerplate. To achieve this we execute the command

```bash
sudo bpftool gen skeleton hello-world.bpf.o > hello-world.skel.h
```

Now we have everything ready and can start writing the user-space program.

```c
#include "hello-world.skel.h"
#include "events.h"
#include <stdio.h>

// print function to be registered with the libbpf library, which will print out information when the eBPF
// binary `hello-world.bpf.o` is loaded into the kernel.
...
int main(int argc, char **arv) {
    ...
    // Defined in the generated boilerplate `hello-world.skel.h`.
...
Since the generated template file is quite large, I have decided not to show the contents of this file, but it is possible to view it at <TODO: link to skeleton file.>.
...
$ clang hello-world.user.c -L/usr/lib64/ -lbpf -lelf -Wl, -rpath /usr/lib64/
$ sudo ./a.out 
...
The reference for the code is available at: <TODO: add link>
...
# Uprobes, Kprobes, Tracepoints etc...
TODO: mention all the above
